...
| DataNode 报 "Cannot start secure DataNode due to incorrect config"（登录已成功） | checkSecureConfig 要求 jsvc 特权端口或 HTTPS_ONLY+sasl resolver；HA 版全链路 HTTP_ONLY → 已加 `dfs.ignore.secure.ports.for.testing=true`（官方测试逃生口，Q14） |
